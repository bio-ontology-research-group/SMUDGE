...
data_dir = '../../../Documents/smudge_updated_data/'
...
with open(data_dir+'gdas_human.dict','r') as f:
	disease_genes = json.load(f)


data = pd.read_csv('data/E_vec_embeddings_human.txt', header = None, sep = ' ', dtype = 'str')
data = data.values
...
disease_set = np.array(disease_genes.keys())

dis_dict = {}
kf = KFold(n = len(disease_set), n_folds = 5, shuffle = True)
for train_idx, test_idx in kf:
	x_train, x_test = disease_set[train_idx], disease_set[test_idx]
...
print('Number of Disease E-Vec human: {}'.format(len(dis_dict)))
